...
func ListFonts(fileName string) {
	...
	if err != nil {
		logger.Fatal("Could not open", fileName, "for writing")
	}
	defer file.Close()
	...
	table.Render()
	logger.Trace("Font list written to", fileName)
}
...
func (options ParsedArgs) ProcessBefore() bool {
	if options.singleInst {
		...
		if err != nil {
			logger.Debug("No instance found or ipc client creation failed:", err)
			return false
		}
		defer client.Close()
		if options.file != "" {
